...
def inference(images):
    ...
    with tf.name_scope('conv1') as scope:
        kernel = tf.Variable(tf.compat.v1.truncated_normal([11, 11, 3, 64],
    ...
    with tf.name_scope('conv2') as scope:
        kernel = tf.Variable(tf.compat.v1.truncated_normal([5, 5, 64, 192],
                                                 dtype=tf.float32, stddev=1e-1, name='weights'))
    ...
    with tf.name_scope('conv3') as scope:
        kernel = tf.Variable(tf.compat.v1.truncated_normal([3, 3, 192, 384],
                                                 dtype=tf.float32, stddev=1e-1, name='weights'))
    ...
    with tf.name_scope('conv4') as scope:
        kernel = tf.Variable(tf.compat.v1.truncated_normal([3, 3, 384, 256],
                                                 dtype=tf.float32, stddev=1e-1, name='weights'))
    ...
    with tf.name_scope('conv5') as scope:
        kernel = tf.Variable(tf.compat.v1.truncated_normal([3, 3, 256, 256],
                                                 dtype=tf.float32, stddev=1e-1, name='weights'))
...
def run_benchmark():
    with tf.Graph().as_default():
        image_size = 224
        images = tf.Variable(tf.compat.v1.random_normal([batch_size,
                                               image_size,
                                               image_size,
                                               3],
                                              dtype=tf.float32,

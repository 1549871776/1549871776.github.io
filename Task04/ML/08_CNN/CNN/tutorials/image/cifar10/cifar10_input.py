...
def read_cifar10(filename_queue):
  ...
  record_bytes = tf.compat.v1.decode_raw(value, tf.uint8)
...
def distorted_inputs(data_dir, batch_size):
  ...
  with tf.name_scope('data_augmentation'):
    ...
    distorted_image = tf.compat.v1.random_crop(reshaped_image, [height, width, 3])
...
def inputs(eval_data, data_dir, batch_size):
  ...
  with tf.name_scope('input'):
    ...
    resized_image = tf.compat.v1.image.resize_image_with_crop_or_pad(reshaped_image,
                                                           height, width)

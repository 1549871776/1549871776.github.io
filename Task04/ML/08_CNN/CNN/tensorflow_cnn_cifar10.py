...
batch_size = 128
# 下载cifar10数据集的默认路径
data_dir = r'G:\ML\尚学堂\08_CNN\CNN\cifar10_data\cifar-10-batches-bin'


def variable_with_weight_losses(shape, stddev, w1):
    ...
    if w1 is not None:
        ...
        # 我们使用w1来控制L2 loss的大小
        weight_loss = tf.multiply(tf.compat.v1.nn.l2_loss(var), w1, name='weight_loss')  # w1是惩罚系数
...
# 生成测试数据，不过这里不需要进行太多处理，不需要对图片进行反转或修改亮度，对比度，不顾需要裁剪图片正中间的24*24大小的区块
# 并进行数据标准化操作
...
# 初始设置一个卷积层，64个卷积核，卷积核大小是5*5，3通道
weight1 = variable_with_weight_losses(shape=[5, 5, 3, 64], stddev=5e-2, w1=0.0)
kernel1 = tf.compat.v1.nn.conv2d(image_holder, filters=weight1, strides=[1, 1, 1, 1], padding='SAME')
bias1 = tf.Variable(tf.constant(0.0, shape=[64]))
conv1 = tf.compat.v1.nn.relu(tf.compat.v1.nn.bias_add(kernel1, bias1))
# 使用尺寸3*3步长2*2的最大池化层层理数据，这里最大池化的尺寸和步长不一样，可以增加数据的丰富性
pool1 = tf.compat.v1.nn.max_pool(conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME')
# 使用LRN对结果进行处理
# LRN最早见于Alex那篇用Ccompat.v1.nn参加ImageNet比赛的论文，Alex在论文中解释LRN层模仿了生物神经系统的"侧抑制"机制，
# 对局部神经元的活动创建竞争环境，使得其中响应比较大的值变得相对更大，并抑制其他反馈较小的神经元，增强了模型的泛化能力
# Alex在ImageNet数据集上的实验表明，使用LRN后Ccompat.v1.nn在Top1的错误率可以降低1.4%，因此其在经典AlexNet中使用了LRN层
# LRN对ReLU这种没有上限边界的激活函数会比较有用，因为它会从附近的多个卷积核的响应中挑选比较大的反馈
# 但不适合Sigmoid这种有固定边界并且能抑制过大值得激活函数
norm1 = tf.compat.v1.nn.lrn(pool1, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75)

# 创建第二个卷积层
weight2 = variable_with_weight_losses(shape=[5, 5, 64, 64], stddev=5e-2, w1=0.0)
kernel2 = tf.compat.v1.nn.conv2d(norm1, filters=weight2, strides=[1, 1, 1, 1], padding='SAME')
# 还有这里的bias值全部初始化为0.1，而不是0，最后,调换了最大池化层和LRN层的顺序，先进行LRN层处理，再使用最大池化层
bias2 = tf.Variable(tf.constant(0.1, shape=[64]))
conv2 = tf.compat.v1.nn.relu(tf.compat.v1.nn.bias_add(kernel2, bias2))
norm2 = tf.compat.v1.nn.lrn(conv2, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75)
pool2 = tf.compat.v1.nn.max_pool(norm2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME')
...
# 接着初始化权值，隐含节点384个，正态分布的标准差设为0.04，bias的值也初始化为0.1
# 注意这里我们希望这全连接层不要过拟合，因此设置了一个非零的weight loss值0.04，让这一层具有L2正则约束
weight3 = variable_with_weight_losses(shape=[dim, 384], stddev=0.04, w1=0.004)
bias3 = tf.Variable(tf.constant(0.1, shape=[384]))
# 最后我们依然使用ReLU激活函数进行非线性化
local3 = tf.compat.v1.nn.relu(tf.matmul(reshape, weight3) + bias3)

# 接下来还是全连接层，只是隐含节点只有一半，其它一样
weight4 = variable_with_weight_losses(shape=[384, 192], stddev=0.04, w1=0.004)
bias4 = tf.Variable(tf.constant(0.1, shape=[192]))
local4 = tf.compat.v1.nn.relu(tf.matmul(local3, weight4) + bias4)
...
weight5 = variable_with_weight_losses(shape=[192, 10], stddev=1/192.0, w1=0.0)
bias5 = tf.Variable(tf.constant(0.0, shape=[10]))
logits = tf.compat.v1.nn.relu(tf.matmul(local4, weight5) + bias5)


# 到这里就完成了整个网络inference的部分，梳理整个网络结构，设计性能良好的Ccompat.v1.nn是有一定规律可循的，但是想要针对某个问题设计最合适的
# 网络结构，是需要大量实际摸索的
# 完成模型inference的构建，接下来是计算Ccompat.v1.nn的loss，这里依然是用cross_entropy，这里我们把softmax的计算和cross_entropy的计算
# 合在了一起，即 tf.compat.v1.nn.sparse_softmax_cross_entropy_with_logits()
...
# 使用 tf.compat.v1.nn.in_top_k()函数求输出结果中 top k的准确率，默认使用top 1，也就是输出分数最高的那一类的准确率
top_k_op = tf.compat.v1.nn.in_top_k(logits, label_holder, 1)
...
num_iter = int(math.ceil(num_examples / batch_size))  # math.ceil()取整函数
true_count = 0
